...
import TenantRentalPayments from "./_components/TenantRentalPayments";

const Financial = () => {
  const [activeTab, setActiveTab] = useState("tenant-rental-payments");
      ...
      >
        ...
        {tabs.map((tab, index) => (
          <TabsContent key={index} value={tab.value}>
            {activeTab === tab.value && (
              <>
                {tab.value === "tenant-rental-payments" && (
                  <TenantRentalPayments />
                )}
                {tab.value === "withdrawals" && <Withdrawals />}
              </>
            )}
          </TabsContent>
        ))}
